...
func TestStrategyTraits(t *testing.T) {
	...
	t.Run("description=not authorized to call endpoints without a session", func(t *testing.T) {
		...
		for k, tc := range []*http.Request{
			httpx.MustNewRequest("POST", publicTS.URL+settings.PublicPath, strings.NewReader(url.Values{"foo": {"bar"}}.Encode()), "application/x-www-form-urlencoded"),
			httpx.MustNewRequest("POST", publicTS.URL+settings.PublicPath, strings.NewReader(`{"foo":"bar"}`), "application/json"),
		} {
